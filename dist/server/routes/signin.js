...
var _openid = _interopRequireDefault(require("../lib/signin/openid"));
...
function _signin() {
  _signin = _asyncToGenerator(function* (req, res) {
    ...
    if (provider.type === 'oauth' && req.method === 'POST') {
      ...
      try {
        var openIdSigninUrl = yield (0, _openid.default)(provider);
        return res.redirect(openIdSigninUrl);
      } catch (error) {
        _logger.default.error('SIGNIN_OPENID_ERROR', error);

        return res.redirect("".concat(baseUrl).concat(basePath, "/error?error=openIdSignin"));
      }
    }
