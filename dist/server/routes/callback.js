...
var _callback3 = _interopRequireDefault(require("../lib/openid/callback"));
    ...
    if (provider.type === 'oauth') {
      ...
      try {
        if (!provider.mapIdentifierToProfile) {
          _logger.default.error('CALLBACK_OPENID_HANDLER_ERROR', 'Must define an mapIdentifierToProfile() handler to use openid authentication provider');
        ...
        try {
          ({
            account: _account3,
            profile: _profile2
          } = yield (0, _callback3.default)(req, provider));
        } catch (error) {
          _logger.default.error('CALLBACK_OPENID_ERROR', error);

          return res.redirect("".concat(baseUrl).concat(basePath, "/error?error=openIdCallback"));
        }

        try {
          var _userOrProfile = _profile2;

          if (adapter) {
            var {
              getUserByProviderAccountId: _getUserByProviderAccountId
            } = yield adapter.getAdapter(req.options);

            var _userFromProviderAccountId = yield _getUserByProviderAccountId(provider.id, _account3.id);

            if (_userFromProviderAccountId) {
              _userOrProfile = _userFromProviderAccountId;
            }
          }

          try {
            var _signInCallbackResponse3 = yield callbacks.signIn(_userOrProfile, _account3, {
              email: _profile2.email
            });

            if (_signInCallbackResponse3 === false) {
              return res.redirect("".concat(baseUrl).concat(basePath, "/error?error=AccessDenied"));
            }
          } catch (error) {
            if (error instanceof Error) {
          ...
          var {
            user: _user3,
            session: _session2,
            isNewUser: _isNewUser2
          } = yield (0, _callbackHandler.default)(sessionToken, _profile2, _account3, req.options);

          if (useJwtSession) {
            var _defaultJwtPayload3 = {
              name: _user3.name,
              email: _user3.email,
              picture: _user3.image
            };

            var _jwtPayload3 = yield callbacks.jwt(_defaultJwtPayload3, _user3, _account3, _profile2, _isNewUser2);

            var _newEncodedJwt3 = yield jwt.encode(_objectSpread(_objectSpread({}, jwt), {}, {
              token: _jwtPayload3
            }));

            var _cookieExpires3 = new Date();

            _cookieExpires3.setTime(_cookieExpires3.getTime() + sessionMaxAge * 1000);

            cookie.set(res, cookies.sessionToken.name, _newEncodedJwt3, _objectSpread({
              expires: _cookieExpires3.toISOString()
            }, cookies.sessionToken.options));
          } else {
            cookie.set(res, cookies.sessionToken.name, _session2.sessionToken, _objectSpread({
              expires: _session2.expires || null
            }, cookies.sessionToken.options));
          }
          ...
          if (error.name === 'AccountNotLinkedError') {
            return res.redirect("".concat(baseUrl).concat(basePath, "/error?error=OpenIdAccountNotLinked"));
          } else if (error.name === 'CreateUserError') {
            return res.redirect("".concat(baseUrl).concat(basePath, "/error?error=OpenIdCreateAccount"));
          } else {
            _logger.default.error('OPENID_CALLBACK_HANDLER_ERROR', error);

            return res.redirect("".concat(baseUrl).concat(basePath, "/error?error=Callback"));
          }
        }
      } catch (error) {
        _logger.default.error('OPENID_CALLBACK_ERROR', error);
