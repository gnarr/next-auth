  ...
  _oAuthCallback = _asyncToGenerator(function* (req) {
    ...
    if ((_provider$version = provider.version) !== null && _provider$version !== void 0 && _provider$version.startsWith('2.')) {
      ...
      try {
        var {
          ...
          refreshToken,
          results
        } = yield client.getOAuthAccessToken(code, provider, pkce.code_verifier);
        var tokens = {
          accessToken,
          refreshToken,
        ...
        if (provider.idToken) {
          if (!(results !== null && results !== void 0 && results.id_token)) {
            throw new _errors.OAuthCallbackError();
          }

          profileData = decodeIdToken(results.id_token);
        } else {
          profileData = yield client.get(provider, accessToken, results);
        }
    ...
    try {
      ...
      var {
        ...
        results: _results
      } = yield client.getOAuthAccessToken(oauthToken, null, oauthVerifier);

      var _profileData = yield client.get(provider.profileUrl, _accessToken, _refreshToken);

      var _tokens = {
        accessToken: _accessToken,
        refreshToken: _refreshToken,
        idToken: _results.id_token
      };
      return _getProfile({
...
function _getProfile(_x2) {
  return _getProfile2.apply(this, arguments);
}

function _getProfile2() {
  _getProfile2 = _asyncToGenerator(function* (_ref) {
    var {
      profileData,
      tokens: {
        accessToken,
...
function decodeIdToken(idToken) {
  ...
  return (0, _jsonwebtoken.decode)(idToken, {
    json: true
  });
}